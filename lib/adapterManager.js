...
function validateDataSourceProperties(adapter,dataSourceProperties){
  if(!dataSourcePropertiesModels[adapter.name]){
      dataSourcePropertiesModels[adapter.name] = mongoose.model('Datasource Adapter properties',adapter.dataSourcePropertiesSchema);
  }
...
function validateQueryProperties(adapter,queryProperties){
  if(!queryPropertiesModels[adapter.name]){
      queryPropertiesModels[adapter.name] = mongoose.model('Query Adapter properties',adapter.queryPropertiesSchema);
  }
